...
const menu = () => {
  return inquirer
    ...
    .then((answers) => {
      //   answers.managerArr.push(answers);
...
const addToEmployeeList = (info) => {
  if (info.employee == "manager") {
    fs.readFile("./lib/Manager.json", "utf8", (err, data) => {
      if (err) {
        console.error(err);
      } else {
        fs.writeFile(
  ...
  if (info.employee == "engineer") {
    fs.readFile("./lib/Engineer.json", "utf8", (err, data) => {
      if (err) {
        console.log(err);
      }
      if (data.length == 0) {
  ...
  if (info.employee == "intern") {
    fs.readFile("./lib/Intern.json", "utf8", (err, data) => {
      if (err) {
        console.log(err);
      }
      if (data.length == 0) {
